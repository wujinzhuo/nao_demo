...
# Use Tsinghua mirrors (Debian 13 trixie uses the deb822 .sources format).
# apt kept on http here: ca-certificates is not guaranteed yet, so https would fail.
...
# Use Tsinghua mirror over http: ca-certificates is installed in this same step,
# so https is not yet usable when apt fetches the package lists.
RUN sed -i 's|http://deb.debian.org|http://mirrors.tuna.tsinghua.edu.cn|g; s|https://deb.debian.org|http://mirrors.tuna.tsinghua.edu.cn|g' /etc/apt/sources.list.d/debian.sources \
